...
Так же есть ядро где nvidia точно опробована из этого видео если она не заработает на ядре 5.9.3 которое я собрал , тут все почти пересобрано. https://github.com/Griggorii/linux-image-oem-5.6.0-1020-kernel-ubuntu-x86_64-deb-package это ядро я забраковал оно тяжелее чем 5.9.3 с моим настроенным конфигом. Данный дистрибутив нацелен на безопасность ядро собранно фактически с несобираемым флагом CONFIG_SYSTEM_TRUSTED_KEYS="" заблокированы ssh и ssh2 c gpg по скольку я не понимаю смысла если мне это не надо и tty и так работает , а значит это для серверов или лазейка для кербероса который итак может манипулировать некими потрохами так как при удалении оного то начинает тянуть все на данный момент денег нет выпилить это так что пока так , удалены так же все сертификаты ssl включая геитсовский по скольку это является паролем для управления своего рода лазейкой и опять для сервера по скольку не имеет смысла на десктопе.
