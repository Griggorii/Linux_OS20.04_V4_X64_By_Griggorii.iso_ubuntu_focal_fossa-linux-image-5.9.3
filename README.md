...
Alternative light Chromium OS77 https://github.com/Griggorii/Chromium_OS_77 best speed boot run system shell - жду идей по доработке что бы на ютубе заработал libffmpeg.so надо сделать в opt папке с chromium библиотеками спец линк типа sudo cp libffmpeg.so /opt/chromium && sudo ln -s /opt/chromium ./libs example support youtube html5 video , я предлагаю доработку этой операционной системы потому что я не собираюсь дарить гуглу эту операционную систему и другим компаниям по причине ограничение моих прав на российском сигменте девелопминга на такой платформе как youtube и отчуждение тем самым от моего канала инвестиции и потери прибыли из за руководящего состава youtube в россии по скольку такие вещи являются очень сложно создаваемыми и порой даже в единственном варианте. Этим подключением библиотеки я показываю что мой уровень выше чем обычный билдер и не раз были уже приведенные примеры и по другим димтрибутивам по этому кто на федеральном уровне будет озвучивать проблему адресной помощи разработчику на griggorii@gmail.com тот ускорит положение в сфере разработки каких то новых идей и их реализации. Смогу ли я сделать для него wine? Да смогу. Если яндекс хоть что то из этой системы взял то тем более пусть знает что он втыкает палки в колеса тому на которого взвесил свои пухлые ноги и поехал касаемо оболочки
