...
Alternative light Chromium OS77 https://github.com/Griggorii/Chromium_OS_77 best speed boot run system shell - жду идей по доработке что бы на ютубе заработал libffmpeg.so надо сделать в opt папке с chromium библиотеками спец линк типа sudo cp libffmpeg.so /opt/chromium && sudo ln -s /opt/chromium ./libs либо  sudo ln -s /opt/chromium /opt/chromium/libs example support youtube html5 video , я предлагаю доработку этой операционной системы потому что я не собираюсь дарить гуглу эту операционную систему и другим компаниям по причине ограничение моих прав на российском сигменте девелопминга на такой платформе как youtube и отчуждение тем самым от моего канала инвестиции и потери прибыли из за руководящего состава youtube в россии по скольку такие вещи являются очень сложно создаваемыми и порой даже в единственном варианте. Этим подключением библиотеки я показываю что мой уровень выше чем обычный билдер и не раз были уже приведенные примеры и по другим димтрибутивам по этому кто на федеральном уровне будет озвучивать проблему адресной помощи разработчику на griggorii@gmail.com тот ускорит положение в сфере разработки каких то новых идей и их реализации. Смогу ли я сделать для chromium os 77 wine? Да смогу. Если яндекс хоть что то из этой системы взял то тем более пусть знает что он втыкает палки в колеса тому на которого взвесил свои пухлые ноги и поехал касаемо оболочки. Сама chromium os 77 обладает самым мощным плюсом она загружается за секунду на hdd и за 0.1 секунды на ssd и более быстрых дисках что делает ее быстрее всех существующих в мире операционных систем в скорости загрузки рабочего окружения и его оболочки. Я являюсь самым первым кто начал создавать из хлама по крупицам новый линукс ну и естественно подход просто /lib легче чем /usr/lib/x86_64-linux-gnu/ чем и воспользовалась федора возможно даже просто взяв и перделав chromium os 77 или какие то части этой os по скольку это андроид структура используемая в x86 , а arm это скорее типа 32 битная x86 по скольку можно сделать давным давно x86_64. Тогда на заре еще никто не ожидал что pulseeffects войдет в состав системы по скольку в 16.04 это надо было ставить отдельно snap или flatpak что то из них в pulse же которая ранее не тормозила наблюдается что парметры ; enable-memfd = yes ; rescue-streams = yes еще не существовали так что можно попробовать их за блеклистить в фаиле daemon.conf этим знаком с пробелом # ; enable-memfd = no и вторую # ; rescue-streams = no
